...
def sendEmail(version):
    fromaddr = '460313911@qq.com'
    password = 'ymegzdrsirmscaib'
    toaddrs = ['xinyan621@outlook.com']
    ...
    for i in range(1, continuousBoard.shape[0]):
        ...
        if i>0:
            ...
            for stock,boards in rowData.iteritems():
                ...
                if rowData[stock] >= 2 and stock not in midArmyStokcs and fallBehindCnt <= 10:
                    if len(name) == 3:
                        name += '  '
                    if 'ST' in name:
                        name += ' '
        ...
        if (total7To8)>0:
            explodeRate = '\n7进8={:.0f}败/{:.0f}成{:.0f}%'\
            .format(fail7To8, success7To8, success7To8/total7To8)
            stockName += explodeRate
        total6To7 = fail6To7+success6To7
        if (total6To7)>0:
            explodeRate = '\n6进7={:.0f}败/{:.0f}成{:.0f}%'\
            .format(fail6To7, success6To7, success6To7/total6To7)
            stockName += explodeRate
        total5To6 = fail5To6+success5To6
        if (total5To6)>0:
            explodeRate = '\n4进5={:.0f}败/{:.0f}成{:.0f}%'\
            .format(fail5To6, success5To6, success5To6/total5To6)
            stockName += explodeRate
        total4To5 = fail4To5+success4To5
        if (fail4To5+success4To5)>0:
            explodeRate = '\n4进5={:.0f}败/{:.0f}成{:.0f}%'\
            .format(fail4To5, success4To5, success4To5/total4To5)
            stockName += explodeRate
        total3To4 = fail3To4+success3To4
        if (total3To4)>0:
            explodeRate = '\n3进4={:.0f}败/{:.0f}成{:.0f}%'\
            .format(fail3To4, success3To4, success3To4/total3To4)
            stockName += explodeRate   
        total2To3 = fail2To3+success2To3
        if (total2To3)>0:
            explodeRate = '\n2进3={:.0f}败/{:.0f}成{:.0f}%'\
            .format(fail2To3, success2To3, success2To3/total2To3)
            stockName += explodeRate
        total1To2 = fail1To2+success1To2
        if (total1To2)>0:
            explodeRate = '\n1进2={:.0f}败/{:.0f}成{:.0f}%'\
            .format(fail1To2, success1To2, success1To2/total1To2)
            stockName += explodeRate
        if noodleKingCnt>0:
        ...
        noodleKingCnt = 0
    picName = "./output/九阳真经断板版本.png"
    if canJoin:
         picName = "./output/九阳真经连板版本.png"
    plt.savefig(picName) # save as png
    plt.show()
    print('制作完毕!')

    print('策略计算项')
    #print(continuousBoard.T[-3:].T)
    twoBoardsDragons = continuousBoard[continuousBoard==2]
    print('2板')
    for i in range(twoBoardsDragons.shape[0]):
        rowData = twoBoardsDragons.iloc[i]  #注意是前一天的数据
        removeRowString(rowData)
        rowData = rowData.sort_values(ascending=False)
        stocks = []
        for stock,boards in rowData.iteritems():
            if math.isnan(boards):
                continue
            if boards > 2:
                continue
            if boards < 2:
            ...
            stocks.append(stock)
        print('----------------------------------------------')
        print(twoBoardsDragons.index.tolist()[i])
        print(stocks)

    print('-----------龙头----------')    
    sixBoardsDragons = continuousBoard[continuousBoard.highestBoard>=6]['code'].tolist()
    print('6板及以上')
    print(sixBoardsDragons)        

    fourToFiveBoards = continuousBoard[(continuousBoard.highestBoard>=3) & \
                                       (continuousBoard.highestBoard<=5)]['code'].tolist()
    print('3-5板')
    print(fourToFiveBoards)
